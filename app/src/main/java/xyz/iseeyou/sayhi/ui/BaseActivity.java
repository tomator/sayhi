...
public class BaseActivity extends AppCompatActivity {
	...
	public void updateUserLocation(){
		if(App.lastPoint!=null){
			String saveLatitude  = mApplication.getLatitude();
			String saveLongtitude = mApplication.getLongtitude();
			String newLat = String.valueOf(App.lastPoint.getLatitude());
			String newLong = String.valueOf(App.lastPoint.getLongitude());
...
//			ShowLog("newLat ="+newLat+",newLong = "+newLong);
			if(!saveLatitude.equals(newLat)|| !saveLongtitude.equals(newLong)){//只有位置有变化就更新当前位置，达到实时更新的目的
				...
				user.setLocation(App.lastPoint);
				user.setObjectId(u.getObjectId());
				user.update(this,new UpdateListener() {
					@Override
					public void onSuccess() {
						// TODO Auto-generated method stub
						App.getInstance().setLatitude(String.valueOf(user.getLocation().getLatitude()));
						App.getInstance().setLongtitude(String.valueOf(user.getLocation().getLongitude()));
//						ShowLog("经纬度更新成功");
					}
					@Override
