...
import xyz.iseeyou.sayhi.adapter.BlackListAdapter;
			...
			public void onClick(DialogInterface dialogInterface, int userId) {
				...
				userManager.removeBlack(user.getUsername(),new UpdateListener() {
					...
					public void onSuccess() {
						...
						ShowToast("移出黑名单成功");
						//重新设置下内存中保存的好友列表
						App.getInstance().setContactList(CollectionUtils.list2map(BmobDB.create(getApplicationContext()).getContactList()));
					}
